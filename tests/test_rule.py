...
class TestRule(TestCase):
    ...
    @patch('leaf_common.rule_based.rule.Condition', autospec=True)
    @patch('leaf_common.rule_based.condition.random.choice', autospec=True)
    @patch('leaf_common.rule_based.rule.random.randint', autospec=True)
    def test_parse_conditions_true(self, mock_randint, mock_randchoice, mock_condition):
        """
        Verify rule parsing when conditions return True
        """
        rule = self._create_rule(mock_condition, mock_randint, mock_randchoice, True, True)
    ...
    @patch('leaf_common.rule_based.condition.random.choice', autospec=True)
    @patch('leaf_common.rule_based.rule.random.randint', autospec=True)
    def test_parse_conditions_false(self, mock_randint, mock_randchoice, mock_condition):
        """
        Verify rule parsing when conditions return mixture of True and False
        """
        rule = self._create_rule(mock_condition, mock_randint, mock_randchoice, True, False)
    ...
    @staticmethod
    def _create_rule(mock_condition, mock_randint, mock_randchoice, first_condition_true, second_condition_true):
